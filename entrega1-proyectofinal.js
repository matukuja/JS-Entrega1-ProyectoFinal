...
function llenarCarrito(arrayPedidos, catalogoPrecios) {
    ...
    let cuotasElegidas = prompt("Elija la cantidad de cuotas con las que desea abonar el pedido." + "\n" + "1 cuota: 0% de interés" + "\n" + "3 cuotas: " + catalogoPrecios.recargo3Cuotas + "% de interés" + "\n" + "6 cuotas: " + catalogoPrecios.recargo6Cuotas + "% de interés" + "\n" + "12 cuotas: " + catalogoPrecios.recargo12Cuotas + "% de interés")
...
function revisarYModificarPedidos() {
    let stringDePedidos = ""
    let contadorPedidos = 0
    let respuestaUsuario = prompt("¿Desea revisar los pedidos que tiene actualmente en el carrito? (Ingrese 'Si' para aceptar, o cualquier otra cosa para cancelar)")
    while ((respuestaUsuario.toUpperCase() == "SI") && (arrayPedidos.length > 0)) {
        arrayPedidos.forEach((pedidoActual) => {
            contadorPedidos = contadorPedidos + 1
            let cadenaEquipo = ""
            let cadenaProducto = ""
            switch (pedidoActual.equipo) {
                case "1":
                    cadenaEquipo = "Boca Juniors"
                    break
                case "2":
                    cadenaEquipo = "River Plate"
                    break
                case "3":
                    cadenaEquipo = "San Lorenzo"
                    break
                case "4":
                    cadenaEquipo = "Independiente"
                    break
                case "5":
                    cadenaEquipo = "Racing"
                    break
            ...
            switch (pedidoActual.tipoProducto) {
                case "1":
                    cadenaProducto = "Imanes"
                    break
                case "2":
                    cadenaProducto = "Llaveros"
                    break
                default:
                    break
            }
            let stringPedidoActual = "Pedido N° " + contadorPedidos + ": -Equipo: " + cadenaEquipo + " -Tipo de producto: " + cadenaProducto + " -Unidades: " + pedidoActual.unidades + "\n\n"
            stringDePedidos = stringDePedidos + stringPedidoActual
        })
        alert(stringDePedidos)
        let respuestaUsuario2 = prompt("¿Desea quitar algún pedido? (Ingrese 'Si' para aceptar, o cualquier otra cosa para cancelar)")
        if (respuestaUsuario2.toUpperCase() == "SI") {
            let pedidoABorrar = parseInt(prompt("Ingrese el número de pedido que desea borrar del carrito: \n" + stringDePedidos))
            arrayPedidos.splice((pedidoABorrar - 1), 1)
            alert("Pedido eliminado correctamente")
        }
        respuestaUsuario = prompt("¿Desea revisar nuevamente los pedidos que tiene actualmente en el carrito? (Ingrese 'Si' para aceptar, o cualquier otra cosa para cancelar)")
        contadorPedidos = 0
        stringDePedidos = ""
    }
}
...
function filtrarPedidos() {
    let respuestaUsuario = prompt("¿Desea filtrar los pedidos que tiene actualmente en el carrito por equipo o por tipo de producto? (Ingrese 'Si' para aceptar, o cualquier otra cosa para cancelar)")
    let arrayFiltrado = []
    while (respuestaUsuario.toUpperCase() == "SI") {
        let criterioDeFiltro = prompt("Indique el número del criterio de filtro por favor: \n" + "1 - Pedidos de Boca Juniors\n" + "2 - Pedidos de River Plate\n" + "3 - Pedidos de San Lorenzo\n" + "4 - Pedidos de Independiente\n" + "5 - Pedidos de Racing\n" + "6 - Pedidos de Imanes\n" + "7 - Pedidos de Llaveros")
        if (criterioDeFiltro > 5) {
            if (criterioDeFiltro == 6) {
                arrayFiltrado = arrayPedidos.filter((elemento) => elemento.tipoProducto == "1")
            } else {
                arrayFiltrado = arrayPedidos.filter((elemento) => elemento.tipoProducto == "2")
            }
        } else {
            arrayFiltrado = arrayPedidos.filter((elemento) => elemento.equipo == criterioDeFiltro)
        }
        if (arrayFiltrado.length > 0) {
            let contadorPedidosFiltrados = 0
            let cadenaResultadoFiltro = ""
            for (const pedidoIterado of arrayFiltrado) {
                let cadenaEquipo = ""
                let cadenaProducto = ""
                switch (pedidoIterado.equipo) {
                    case "1":
                        cadenaEquipo = "Boca Juniors"
                        break
                    case "2":
                ...
                switch (pedidoIterado.tipoProducto) {
                    case "1":
                        cadenaProducto = "Imanes"
                        break
                    case "2":
                ...
                contadorPedidosFiltrados = contadorPedidosFiltrados + 1
                cadenaResultadoFiltro = cadenaResultadoFiltro + "Resultado N° " + contadorPedidosFiltrados + ": -Equipo: " + cadenaEquipo + " -Tipo de producto: " + cadenaProducto + " -Unidades: " + pedidoIterado.unidades + "\n"
            }
            alert(cadenaResultadoFiltro)
        } else {
            alert("No hay pedidos que apliquen al filtro especificado por usted.")
        }
        respuestaUsuario = prompt("¿Desea filtrar los pedidos que tiene actualmente en el carrito por equipo o por tipo de producto? (Ingrese 'Si' para aceptar, o cualquier otra cosa para cancelar)")
    }
}
...
if (usuarioCompro()) {
    alert("Felicitaciones! Sus pedidos fueron incorporados correctamente al carrito de compras!")
        //le doy al usuario la opción de revisar su pedido y modificarlo en caso de que así lo desee
    revisarYModificarPedidos()
    if (usuarioCompro()) {
        //le doy al usuario la opción de filtrar sus pedidos por equipo (cualquiera de los 5) o tipo de producto (imanes o llaveros), para que pueda ver lo que está
        //llevando antes de crear el carrito de compras definitivo
        filtrarPedidos()
        ...
        alert(clienteActual.devolverNombreYApellido() + "(Doc. N° " + clienteActual.dni + "), muchas gracias por su compra. Su pedido será despachado en las próximas semanas a la dirección '" + clienteActual.direccion + "'. En breve le llegará el link de seguimiento al teléfono de contacto '" + clienteActual.telContacto + "'.")
    } else {
